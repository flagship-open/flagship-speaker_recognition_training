...
def get_parser():
    ...
    parser.add_argument('--lr_schedule', help='Number of epochs for learning rate piecewise.', default=[120,240,360])
    ...
    parser.add_argument('--margin_m', type=float,
                        help='insightface_loss/cosineface_losses/combine_loss loss margin.', default=0.35)
...
if __name__ == '__main__':
    with tf.Graph().as_default():
        ...
        train_vic = np.expand_dims(train_vic, axis=1)
        # print(len(train_img.get_shape.as_list()))
        # exit()

        # identity the input, for inference
        inputs_img = tf.identity(inputs_img, 'input_img')
        inputs_vic = tf.identity(inputs_vic, 'input_vic')
        prelogits = inference(inputs_img, inputs_vic, phase_train=phase_train_placeholder, weight_decay=args.weight_decay)
        ...
        learning_rate = tf.train.piecewise_constant(epoch, boundaries=args.lr_schedule, values=[0.1, 0.01, 0.001, 0.0001],
                                         name='lr_schedule')
        ...
        for i in range(args.max_epoch):
            for idx in range(start_batch_id, train_iteration):
                voices_train = train_vic[idx * args.train_batch_size:(idx + 1) * args.train_batch_size]
                images_train = train_img[idx * args.train_batch_size:(idx + 1) * args.train_batch_size]
                labels_train = label[idx * args.train_batch_size:(idx + 1) * args.train_batch_size]

                _ = sess.run(inc_epoch_op)


                feed_dict = {inputs_img: images_train, inputs_vic: voices_train, labels: labels_train, phase_train_placeholder: True}
                start = time.time()
