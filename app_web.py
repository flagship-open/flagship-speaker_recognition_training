...
ns = api.namespace('Identification_Request', description='Multi Modal 화자 인식 결과')
app.config.SWAGGER_UI_DOC_EXPANSION = 'full'
model_input = api.model('input',{'name':fields.String(required=True, description='mp4 파일 이름'),
                                 'path_dir':fields.String(required=True, description='시작경로'),
                                 're_register':fields.String(required=True, description='재등록여부'
                                                             )})
...
class FR(Resource):
    ...
    def post(self):
        ...
        path = request.json['path_dir']
        name = request.json['name']
        a = 'test_data/' + name.split('@')[0]
        name_class = name.split('-')[0:-1]  # aaa@yonsei.ac.kr
        re_register = request.json['re_register']
